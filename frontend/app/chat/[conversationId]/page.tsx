...
export default function ChatPage() {
  ...
  const isProvider = searchParams?.get("as") === "provider";
  ...
  // Poll for new messages (provider sees user replies, user sees provider replies)
  useEffect(() => {
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    const res = await fetch(`/api/v1/messages/conversations/${conversationId}/messages?sender_type=${senderType}`, {
      method: "POST",
  ...
  return (
    <main className="min-h-screen flex flex-col max-w-2xl mx-auto bg-white dark:bg-zinc-950">
      <div className="flex items-center gap-2 p-4 border-b border-zinc-200 dark:border-zinc-800">
        ...
        <span className="font-medium">{isProvider ? "Reply as provider" : "Conversation"}</span>
      </div>
      <div className="flex-1 overflow-y-auto p-4 space-y-4 min-h-[200px]">
        ...
        {messages.length === 0 && (
          <p className="text-zinc-500 text-sm text-center py-8">No messages yet. Send one below.</p>
        )}
        <div ref={bottomRef} />
      </div>
      <form onSubmit={sendMessage} className="p-4 border-t border-zinc-200 dark:border-zinc-800 flex gap-2">
        <input
          ...
          placeholder={isProvider ? "Type your reply…" : "Type a message…"}
          className="flex-1 rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-zinc-900 dark:text-zinc-100 placeholder:text-zinc-500 px-4 py-2"
        />
